...
export const fetchAllTasksService = async(userId, filters) => {
    ...
    try{
        ...
        {
            whereClause.dueDate = {
                [Op.between]: [new Date(dueDateStart), new Date(dueDateEnd)] //get all dueDates between this range
            };
        }
        else if (dueDateStart) //when only dueDateStart is provided
        {
            whereClause.dueDate = {[Op.gte]: new Date(dueDateStart)}; // get all due dates greater than dueDateStart
        }
        else if (dueDateEnd) //when only dueDateEnd is provided
        {
            whereClause.dueDate = {[Op.lte]: new Date(dueDateEnd)}; // get all due dates less than dueDateEnd
        }
        ...
        if(sortBy && (sortBy==='dueDate' || sortBy==='priority') && (sortOrder.toUpperCase === 'ASC' || sortOrder.toUpperCase === 'DESC'))
        {
